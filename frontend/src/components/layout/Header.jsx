    ...
    >
      ...
      <div className={styles.center}>
        <div className={styles.antennaWrap} aria-hidden="true">
          {/* SVG antenna icon matching the reference */}
      ...
      <div className={styles.right}>
        <PillStat
          icon={<BsFire size={16} color="var(--c-orange)" />}
          label="Streak"
          value={streak}
          valueClass={styles.valOrange}
        />
        <PillStat
          icon={<LuTrophy size={16} color="var(--c-yellow)" />}
          label="Score"
          value={score.toLocaleString()}
          valueClass={styles.valYellow}
        />
        <button className={styles.profile} aria-label="Profile menu">
          <FaUserCircle size={22} color="var(--c-muted)" />
          <FiChevronDown size={14} color="var(--c-muted)" />
        </button>
      </div>
    </motion.div>
  );
